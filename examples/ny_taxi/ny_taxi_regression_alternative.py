...
from inhabitation_task import LuigiCombinator, ClsParameter, RepoMeta, InhabitationTask, TaskState, states, ConfigIndex
from cls_python import FiniteCombinatoryLogic, Subtypes, Type
...
class PreprocessTabularData(luigi.Task, LuigiCombinator):
    ...
    def run(self):
        if self.config_index == PreprocessorStates.active:
            self.run_active()
...
class PreprocessDateFeature1(PreprocessTabularData):
    abstract = False
    file_name = luigi.Parameter(default="data/tabular_data_preprocessed1.pkl")
    ...
    def run_active(self):
        ...
        tabular.to_pickle(self.file_name)


class PreprocessDateFeature2(PreprocessTabularData):
    abstract = False
    file_name = luigi.Parameter(default="data/tabular_data_preprocessed2.pkl")
...
class PreprocessorSet(luigi.Task, LuigiCombinator):
    ...
    def active_requirements(self, *args):
        return [self.tabular_data(), *(arg for arg in args if arg.config_index == PreprocessorStates.active)]
...
class AnyPreprocessor(PreprocessorSet):
    abstract = False
    date_feature1 = ClsParameter(tpe=PreprocessDateFeature1.return_type(PreprocessorStates.inactive))
    date_feature2 = ClsParameter(tpe=PreprocessDateFeature2.return_type(PreprocessorStates.inactive))

    def requires(self):
        return self.active_requirements(self.date_feature1(), self.date_feature2())
