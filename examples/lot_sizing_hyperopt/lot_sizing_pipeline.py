...
class HyperOptimizedTask(luigi.Task):
    update_mode = luigi.BoolParameter()
    trials = luigi.Parameter(default=None)
    trial_index = luigi.IntParameter(default=0)

    def actual_output(self):
        pass

    def actual_run(self, optimized_requirements):
        pass

    def requires_optimized():
        pass

    def last_updated_trial_index(self):
        pass

    def output(self):
        if self.update_mode:
            return luigi.LocalTarget(f'trials/{self.task_id()}')
        else:
            return self.actual_output()

    def _update(self):
        last_updated = self.last_updated_trial_index()
        for requirement in self.requires_optimized():
            updated_requirement = yield requirement(update_mode=True, trials=self.trials, trial_index=self.trial_index)
            with updated_requirement.open('r') as f:
                requirement_index = int(next(f))
                if requirement_index >= last_updated:
                    last_updated = requirement_index
        with self.output().open('w') as f:
            f.write(str(last_updated))

    def _optimized_requirements(self):
        optimized_requirements = list()
        for requirement in self.requires_optimized():
            # taken from luigi cache as update was called at this point
            updated_requirement = yield requirement(update_mode=True, trials=self.trials, trial_index=self.trial_index)
            with updated_requirement.open('r') as f:
                requirement_index = int(next(f))
                optimized_requirements.append(requirement(update_mode=False, trials=self.trials[:requirement_index+1], trial_index=requirement_index))
    ...
    def run(self):
        if self.update_mode:
            ...
            # cached result of computing the update index
            updated_self = yield self.clone(update_mode=True)
            with updated_self.output().open('r') as f:
                updated_index = int(next(f))
                if (updated_index >= self.trial_index):
                    self.actual_run(self._optimized_requirements())
                else:
                    # cached prior result
                    yield self.clone(trials=self.trials[:updated_index+1], trial_index=updated_index)
...
class PredictDemand(HyperOptimizedTask, LuigiCombinator):
    abstract = True
