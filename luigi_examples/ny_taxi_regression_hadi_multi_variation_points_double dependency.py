...
class AddColTestIfPickupIsInWeekendEncoded(luigi.Task, LuigiCombinator):
    abstract = False
    ...
    def _read_tabular_data(self):
        return pd.read_pickle(self.input().path)

    def run(self):
        tabular = self._read_tabular_data()
        tabular['Test'] = 99999999
        tabular.to_pickle(self.output().path)

    def output(self):
        return luigi.LocalTarget('data/Test.pkl')
...
class DummyEncodingNode(AddColTestIfPickupIsInWeekendEncoded,
...
s = {AddColTestIfPickupIsInWeekendEncoded,
     EncodePickupWeekdayOneHotSklearn}
...
class ExtractFeatures(luigi.WrapperTask, LuigiCombinator):
    ...
    double_dependency = ClsParameter(tpe=AddColTestIfPickupIsInWeekendEncoded.return_type() \
        if AddColTestIfPickupIsInWeekendEncoded in s else DummyEncodingNode.return_type())

    def requires(self):
        return [self.filtered_trips(), self.raw_temporal_features(), self.is_after_7am(7),
                self.onehot_weekdays(), self.double_dependency()]
    def output(self):
        return self.input()
