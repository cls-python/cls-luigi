...
class GetBestResult(CLSTask, globalConfig):
    abstract = False
    count = 0
    luigi_count = luigi.IntParameter(default=1, batch_method=max)
    hash_map_task = ClsParameter(tpe=CreateHashMapResult.return_type())


    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.best_result = luigi.LocalTarget(pjoin(str(self.hash_map_result_path), "best_result.txt"))
        self.luigi_count = self.next_count()

    def requires(self):
        return self.hash_map_task()

    def output(self):
        return {"best_result" : self.best_result}

    def run(self):
         with open(self.output()["best_result"].path, "a") as best_result_file:
            best_result_file.write(str(self.luigi_count) + " /n ")
...
if __name__ == '__main__':
    target = GetBestResult.return_type()
