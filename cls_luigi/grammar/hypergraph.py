...
def get_hypergraph_dict_from_tree_grammar(
    ...
    for lhs, rhs in tree_grammar.items():
        ...
        if lhs not in hypergraph["type_nodes"]:
            hypergraph["type_nodes"].append(lhs)

        for combinator, args in rhs.items():

            if combinator not in hypergraph["subtype_nodes"]:
                hypergraph["subtype_nodes"].append(combinator)

            hypergraph["choice_edges"].append((lhs, combinator))
            for arg in args:
                if arg not in hypergraph["type_nodes"]:
                    hypergraph["type_nodes"].append(arg)
...
def plot_hypergraph_components(
    ...
    all_nodes = hypergraph["type_nodes"] + hypergraph["subtype_nodes"]
    ...
    nx.draw_networkx_nodes(g, pos, node_size=node_size, nodelist=hypergraph["type_nodes"],
                           ax=axs, node_color=type_nodes_color, node_shape='s')

    nx.draw_networkx_nodes(g, pos, node_size=node_size, nodelist=hypergraph["subtype_nodes"],
                           ax=axs, node_color=subtype_nodes_color, node_shape='o')
