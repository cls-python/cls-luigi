...
def update_tasks_status(stop_event):
    print("Start Task-Status Updater\n")
    ...
    while keep_updating and not stop_event.is_set():
        try:
            if os.path.exists(dynamic_repo_name):
                ...
                luigi_task_updates = requests.get(
                    "http://localhost:8082/api/task_list"
                ).json()["response"]
                tasks_status = set()
                ...
                for k in loaded.keys():
                    for j in loaded[k].keys():
                        if loaded[k][j]["luigiName"] in luigi_task_updates:
                            loaded[k][j]["status"] = luigi_task_updates[
                                loaded[k][j]["luigiName"]
                            ]["status"]
                            loaded[k][j]["timeRunning"] = luigi_task_updates[
                                loaded[k][j]["luigiName"]
                            ]["time_running"]
                            loaded[k][j]["startTime"] = luigi_task_updates[
                                loaded[k][j]["luigiName"]
                            ]["start_time"]
                            loaded[k][j]["lastUpdated"] = luigi_task_updates[
                                loaded[k][j]["luigiName"]
                            ]["last_updated"]
                            tasks_status.add(
                                luigi_task_updates[loaded[k][j]["luigiName"]]["status"]
                            )
