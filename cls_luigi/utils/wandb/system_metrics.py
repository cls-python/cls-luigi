...
def start_logging_metrics():
    ...
    log_thread = threading.Thread(target=_log_system_metrics, args=(process,), daemon=True)
    log_thread.start()
            ...
            wandb_log(
                {
                    ...
                    "gpu_metrics_nvidia": gpu_metrics.get('NVIDIA', None),
                    "gpu_metrics_amd": gpu_metrics.get('AMD', None)
                },
                data_type="metrics"
            )
